...
fw.write('id^title^url^cover^rate^director^composer^actor^category^district^language^showtime^length^othername^\n')
...
for line in fr:
	...
	for item in category:
		if not item.strip() in categories:
			continue
		temp = temp + item.strip() + '/'
	category = temp[:-1]
	if len(category) == 0:
	...
	record = movieId + '^' + title + '^' + url + '^' + cover + '^' + rate + '^' + director + '^' + composer + '^' + actor + '^' + category + '^' + district + '^' + language + '^' + showtime + '^' + length + '^' +othername+'^'+ '\n'
	# print(record)
	fw.write(record)
